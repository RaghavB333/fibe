...
const NavbarForMobile = () => {
    ...
    return (
        <>
            {/* Navbar */}
            <nav className='flex justify-between items-center px-8 bg-gradient-to-t from-[#FFF1F5] to-[#FFF8ED] shadow-lg z-10 relative'>
            ...
            {showHamburger && (
                <>
                    ...
                    <div
                        ...
                        className={`fixed inset-0 bg-black z-40 transition-opacity duration-300 ease-in-out ${animateSidebar ? 'opacity-40' : 'opacity-0'}`}
                    ></div>

                    {/* Sidebar */}
                    <div
                        className={`overflow-y-auto fixed top-0 right-0 h-screen w-[78vw] rounded-l-4xl bg-white z-50 shadow-lg transform transition-transform duration-300 ease-in-out ${animateSidebar ? 'translate-x-0' : 'translate-x-full'}`}
                    >
